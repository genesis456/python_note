...
- 服务器端接收参数
    ...
    2. 获取客户端请求GET请求提交的数据
        1. 语法
            ```python
            request.GET['参数名']  # QueryDict 类似字典，不同的是可以一键多值，返回列表中的值
            
            # #查询某参数的值，若没有返回默认值
            request.GET.get('参数名','默认值')
            
            #返回原列表（request.GET.get是返回列表中的值）
...
- 取消csrf验证,否则Django将会拒绝客户端发来的POST请求
  ...
  - 取消 csrf 验证
    ...
    - 删除（或注释） settings.py 中 MIDDLEWARE 中的 CsrfViewsMiddleWare 的中间件
