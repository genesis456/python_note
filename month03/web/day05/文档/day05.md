...
2. 文档内嵌。使用<script type="text/javascript"></script>标签书写 JavaScript 代码
      ...
      ```
      + 注意 ：<script></script>标签可以书写在文档的任意位置，书写多次，一旦加载到script标签就会立即执行内部的JavaScript代码，因此不同的位置会影响代码最终的执行效果(会按顺序执行下去)
3. 外部链接
      + 创建外部的JavaScript文件 XX.JavaScript，在HTML文档中使用<script src=""></script>引入
      ```html
      <script src="index.JavaScript"></script>
      ```
      + 注意 ：<script></script>既可以实现内嵌 JavaScript 代码，也可以实现引入外部的 JavaScript 文件，但是只能二选一。（不可两个都写）
4. JavaScript 输入和输出语句
      + alert("")；普通的网页弹框
      + prompt("")  接收用户输入的内容，返回用户输入的内容（类似于input)
      + console.log()；控制台输出，多用于代码调试(类似于print)
      + document.write("<h1>Hello</h1>")；实现在动态在网页中写入内容
         1. 可以识别HTML标签,脚本代码可以在文档任何地方书写，如果是普通写入（不涉及事件），区分代码的书写位置插入
         2. **如果用时间绑定的话**，会重写网页内容(**慎用**)
...
2. 使用
   字符串之间比较大小
        ...
        2.非数字字符串之间的比较
            转成ascii码
            'abc' < 'defg'
        3.数字和字符串之间比较大小，且字符串为纯数字字符串
            将数字字符串转换为整数，去比较
            123 < '456'

        4.中文比较
            str.charCodeAt(index)
            返回字符的unicode值  0-65535 之间
            index为索引

        5.当数字和字符串去比较,且字符串为非纯数字字符串:
            将非数字字符串转换为整数
            '1a2b3c' --> NaN
            当NaN和任何数字进行比较时，都会返回false
